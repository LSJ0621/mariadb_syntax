...
-- 조회 시 index를 통해 조회하려면 반드시 where 조건에 해당 컬럼이 있어야함
...
-- 만약에 두 컬럼에 동시에 index가 걸려있다면 해당 index 활용
-- 만약에 두 컬럼 각각 index가 설정돼 있다면 mariadb에서 최적의 알고리즘 실행
select * from author where name = 'hongildong' and address = '서울시'

-- index 제거
alter table author drop index 인덱스명;

-- 데이터 대량 생성 후 조회(100만개)
--  대량 데이터 생성 프로시저 생성
DELIMITER //
